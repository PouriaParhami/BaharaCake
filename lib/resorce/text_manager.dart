class TextManager{
  TextManager._();

  static const String weddingTitle = "Wedding";
  static const String coffeeShopTitle = "CoffeeShop";
  static const String birthdayTitle = "Birthday";
  static const String aboutTitle = "About";
  static const String location = "Map";
  static const String contact = "Contact Us";
  static const String welcomeTitle = "Welcome To BaharaCake";
  static const String homePageShortAbout =
      "BAHARA CAKE team started its home production in 2015 By producing all kinds of traditional and modern cakes and sweets BAHARA CAKE team started with Bahare and Yalda Sadeghi, and today, with a large team, they operate in the world of cakes and sweets professionally and with homemade taste.";
  static const String phoneNUmber = "09151217162";
  static const String email = "baharacake@gmail.com";
  static const String birthdayDescription = "Celebrate Life's Sweetest Moments with Us! Birthdays are for laughter, love, and of course, cake! Let us be a part of your special day, creating confectionery delights that will make you smile from ear to ear. From themed cookies to personalized cakes, we'll turn your dreams into delicious reality. So blow out the candles, make a wish, and let's embark on a sweet journey together! Happy birthday, let's make it unforgettable!";
  static const String coffeeShopDescription =  "Indulge in our Sweet Symphony of Flavors! Take a break from the hustle and bustle and treat yourself to a slice of heaven. Our cookies and cakes are the perfect companions to your favorite brew, elevating your coffee experience to new heights. From classic chocolate chip cookies to decadent layered cakes, each bite is a delicious adventure waiting to be savored. So sit back, sip, and let the sweetness take over!";
  static const String weddingDescription = "Welcome to our Wedding Sweet Spot! Step into a world where love is the main ingredient and every bite tells a story. From elegant tiered cakes to delicate cookies, we're here to add a touch of sweetness to your special day. Explore our gallery of confectionery wonders, crafted with care and sprinkled with joy. Let's create sweet memories together that will last a lifetime!";


}